...
export default function UserResetForm() {
    ...
    const closeModal = () => {
        const modal = document.getElementById("my_modal_userReset");
    ...
    const handleReset = async (dataSet: {email: string}) => {
        try {
            ...
            const response = await fetch(`http://localhost:8000/api/accounts/reset-verify?accountType=user&email=${dataSet.email}`, {
                method: "GET",
                headers: {
